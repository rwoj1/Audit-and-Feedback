...
const CATALOG = {
  ...
  "Benzodiazepines / Z-Drug (BZRA)": {
    ...
    Lorazepam: { Tablet: ["1 mg","2.5 mg"] }, // 0.5 mg removed
...
function buildPacksFromDoseLines(){
  ...
  doseLines.forEach(ln=>{
    const baseMg = parseMgFromStrength(ln.strengthStr); // for oxy/nx this is oxy only
...
function splitDailyPreferred(total, strengthsAsc){
  ...
  for(let amT=half; amT>=0; amT--){
    const pmT=total-amT; const am=composeExact(amT,strengths); if(!am) continue;
    const pm=composeExact(pmT,strengths); if(pm && pmT>=amT) return {AM:am, PM:pm};
  }
  const all=composeExact(total,strengths)||{[strengths[0]]:1};
  return {AM:{}, PM:all};
}
...
function buildPlanTablets(){
  ...
  applyStep(); if(packsTotalMg(packs)>0.0001) rows.push({week,date:fmtDate(date),packs:deepCopy(packs),med,form,cls});
...
function normalizePatchDisplay(med, list){
  ...
  // simplify 12+12 -> 25 for fentanyl
  if(med==="Fentanyl"){
  ...
  // merge equal strengths into single total if matches an available figure
  const sum=used.reduce((x,y)=>x+y,0);
  if(avail.includes(sum)) return [sum];
  return used;
}
...
function buildPlanPatch(){
  ...
  /* start at first reduction (step 2) */
  ...
  const rows=[]; let date=startOfWeek(startDate); let week=1;
  let nextReductionDate = addDays(date, reduceEvery); // when we try next % drop
...
/* choose commercial base preferring larger strength -> fewer pieces */
function perStrengthRowsFractional(r){
  ...
  ["AM","MID","DIN","PM"].forEach(slot=>{
    Object.entries(r.packs[slot]).forEach(([pieceStr, count])=>{
      ...
      for(const b of baseDesc){ // prefer bigger base
...
function renderStandardTable(rows){
  ...
  schedule.appendChild(table);
    ...
    mgList.forEach((mg,i)=>{
      ...
      let strengthLabel = `${r.med} ${(+mg).toString().replace(/\.0+$/,"")} mg ${formLabelCapsSR(r.form)}`;
      if(r.med==="Oxycodone / Naloxone") strengthLabel = oxyNxLabelForOxyMg(+mg);
