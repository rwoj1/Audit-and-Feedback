...
/* =============== utilities =============== */
const $ = (id) => document.getElementById(id); // pass the raw id (no '#')
const fmtDate = (d) =>
...
function banner(msg) {
  ...
  b.style.cssText =
    "position:fixed;left:0;right:0;bottom:0;background:#7f1d1d;color:#fff;padding:8px 12px;font:14px system-ui;z-index:999999";
  b.textContent = msg;
  document.body.appendChild(b);
}
function roundToNearestMultiple(x, step) {
...
/* =============== rules =============== */
function isMR(form) {
  return /slow\s*release|modified|controlled|extended|sustained/i.test(form) ||
         /(\bSR|MR|CR|ER|XR|PR|CD\b)/i.test(form);
}
...
function canQuarter(med, form) { return canHalf(med, form); }

/* =============== parsing & dropdowns =============== */
function parseMgFromStrength(str) {
  if (!str) return 0;
  const lead = String(str).split("/")[0];              // oxycodone/naloxone → oxycodone only
...
function populateClasses() {
  const el = $("classSelect"); if (!el) return;
  el.innerHTML = "";
...
function renderDoseLines() {
  ...
  doseLines.forEach((ln, idx) => {
    ...
    if (form === "Patch") {
      const o = document.createElement("option");
      o.value = "PATCH";
      o.textContent = patchFreqLabel(med); // Fentanyl=3 days, Buprenorphine=7 days
      fSel.appendChild(o);
      fSel.disabled = true;
    } else {
      [
        ["AM","Daily in the morning"],
        ["MID","Daily at midday"],
        ["DIN","Daily at dinner"],
        ["PM","Daily at night"],
        ["BID","Twice daily (morning & night)"],
    ...
    fSel.addEventListener("change", (e) => {
      const id = +e.target.dataset.id;
      const l = doseLines.find((x) => x.id === id);
      if (l) l.freqMode = e.target.value;
    });
...
function updateRecommended() {
  ...
  bp.className = "bp" + (cls === "Opioid" ? " opioid" : "");
  bp.innerHTML = `<strong>Suggested Practice for ${cls || ""}</strong><ul>${
    (RECOMMEND[cls] || []).map(t=>`<li>${t}</li>`).join("")
  }</ul>`;
...
function splitDailyToBID_withPieces(totalDailyMg, pieceSet){
  ...
  for(let amTarget=half; amTarget>=0; amTarget--){
    ...
    if(pm && pmTarget >= amTarget){ return { AM:am, PM:pm }; }
  }
...
function labelStrength(med, mg, form){
  const mgTxt = ( +mg.toFixed(2) ).toString().replace(/\.00$/,'');
  // Ensure SR stays uppercase
  if (/^\s*SR\s*Tablet$/i.test(form) || /Slow\s*Release/i.test(form)) {
    return `${med} ${mgTxt} mg SR tablet`;
  }
...
function opioidStep(packs, percent, med, form){
  ...
  // Build BID using available whole tablets (no splitting for opioids)
  const pieceSet = strengthsForSelected().map(parseMgFromStrength).filter(v=>v>0);
  const bid = splitDailyToBID_withPieces(targetDaily, pieceSet);
...
function bzraOrAntipsychoticStep(packs, percent, med, form){
  ...
  // Force dosing to AM/PM (MID & DIN trimmed first by construction)
  const pieces = allowedPiecesMg(med, form);
...
function buildPlanTablets(){
  ...
  const doOneStep = (percent) => {
    if (cls === "Opioid") {
      packs = opioidStep(packs, percent, med, form);
      ...
      // Reduce MID → AM → PM, end tends to dinner-only via user’s rules
      const total = packsTotalMg(packs);
      let rem = +(total * (percent/100)).toFixed(4);
...
function buildStrengthRowInstructions(mg, packs){
  const lines = [];
  const am = packs.AM[mg] || 0;
  const mid= packs.MID[mg] || 0;
  const din= packs.DIN[mg] || 0;
  const pm = packs.PM[mg] || 0;
...
function renderStandardTable(rows){
  ...
  rows.forEach(r => {
    ...
    lines.forEach((mg, idx) => {
      ...
      const strengthLabel = labelStrength(med, mg, form);
...
function init(){
  ...
  // Start with one dose line
  doseLines = [];
  addDoseLine();

  // Listeners (note: pass raw ids to $)
