...
function qToPhrase(q){ // q in quarters (1=quarter,...,4=one)
...
const CATALOG={
  ...
  "Benzodiazepines / Z-Drug (BZRA)":{
    ...
    Lorazepam:{ Tablet:["1 mg","2.5 mg"] }, /* 0.5 removed */
...
function slotsForFreq(mode){
  switch(mode){ case "AM":return["AM"]; case "MID":return["MID"]; case "DIN":return["DIN"]; case "PM":return["PM"];
...
function renderDoseLines(){
  ...
  doseLines.forEach((ln,idx)=>{
    ...
    if(form==="Patch"){ const o=document.createElement("option"); o.value="PATCH"; o.textContent=(med==="Fentanyl")?"Every 3 days":"Every 7 days"; fSel.appendChild(o); fSel.disabled=true; }
...
const RECOMMEND={
  Opioid:["Tailor plan to clinical characteristics, goals and preferences.","< 3 months: reduce 10–25% weekly.","> 3 months: reduce 10–25% every 4 weeks.","Long-term/high doses: slower taper and frequent monitoring."],
...
function allowedPiecesMg(cls, med, form){
  ...
  if(split.half||split.quarter){ uniq.forEach(v=>{ if(split.half) pieces.push(+(v/2).toFixed(3)); if(split.quarter) pieces.push(+(v/4).toFixed(3)); }); }
...
function composeExactOrLower(target, pieces, step){
  ...
  while(t>0){ const u=composeExact(t,pieces); if(u) return u; t=+(t-step).toFixed(3); }
  return {};
}
...
function removeFromPackByMg(pack, amount){
  let toDrop=+amount.toFixed(3); const sizes=Object.keys(pack).map(parseFloat).sort((a,b)=>b-a);
  for(const p of sizes){ while(toDrop>0 && pack[p]>0){ pack[p]-=1; if(pack[p]===0) delete pack[p]; toDrop=+(toDrop-p).toFixed(3); } if(toDrop<=0) break; }
...
function buildPacksFromDoseLines(){
  ...
  doseLines.forEach(ln=>{
    const baseMg=parseMgFromStrength(ln.strengthStr); // oxy/nx: leading part = oxy mg
...
function splitDailyPreferred(total, strengthsAsc){
  const strengths=strengthsAsc.slice().sort((a,b)=>a-b); const half=Math.floor(total/2);
  for(let amT=half; amT>=0; amT--){ const pmT=total-amT; const am=composeExact(amT,strengths); if(!am) continue; const pm=composeExact(pmT,strengths); if(pm && pmT>=amT) return {AM:am, PM:pm}; }
  const all=composeExact(total,strengths)||{[strengths[0]]:1}; return {AM:{}, PM:all};
}
function opioidOrPpiStep(packs, percent){
  ...
  const step=strengths[0]||1; let target=roundTo(total*(1-percent/100), step);
  if(target===total && total>0){ target=Math.max(0,total-step); target=roundTo(target,step); }
  const bid=splitDailyPreferred(target,strengths); return { AM:bid.AM, MID:{}, DIN:{}, PM:bid.PM };
}
...
/* patches (start at step 2; monotonic down; simplify totals) */
function normalizePatchDisplay(med, list){
  const total=(list||[]).reduce((a,b)=>a+b,0);
  const avail=(med==="Fentanyl")?[100,75,50,25,12]:[40,30,25,20,15,10,5];
  if(avail.includes(total)) return [total];
  let rem=total, used=[];
  for(const s of avail){ while(rem>=s-1e-9){ used.push(s); rem-=s; } }
...
function nextLowerAvail(total, med){
  const avail=(med==="Fentanyl")?[100,75,50,25,12]:[40,30,25,20,15,10,5];
  for(let i=0;i<avail.length;i++){ if(total===avail[i]) return avail[i+1] ?? avail[i]; if(total>avail[i]) return avail[i]; }
...
function buildPlanPatch(){
  ...
  while(true){
    rows.push({ week, date:fmtDate(date), patches: normalizePatchDisplay(med,[current]), med, form });
    ...
    if(hitReview || hit3mo){ rows.push({ week:week+1, date:fmtDate(nextDate), patches:[], med, form, review:true }); break; }

    if(+nextDate >= +nextReductionDate){
      let target=Math.max(smallest, Math.ceil(current*(1 - reducePct/100)));
      if(target===current) target = nextLowerAvail(current, med); // force progress
      if(target<=smallest){
        if(heldSmallest){ rows.push({ week:week+1, date:fmtDate(nextDate), patches:[], med, form, stop:true }); break; }
        heldSmallest=true;
      }
      current=target;
...
// map oxy mg → commercial oxy/nx text
function oxyNxPairLabel(oxyMg){
  const list=(CATALOG?.Opioid?.["Oxycodone / Naloxone"]?.["SR Tablet"])||[];
  const hit=list.find(s=>{ const lead=String(s).split("/")[0]; const m=lead.match(/([\d.]+)\s*mg/i); return m && parseFloat(m[1])===+oxyMg; });
...
function perStrengthRowsFractional(r){
  ...
  const byBase={}; const addQ=(b,slot,q)=>{ byBase[b]=byBase[b]||{AM:0,MID:0,DIN:0,PM:0}; byBase[b][slot]+=q; };
  ["AM","MID","DIN","PM"].forEach(slot=>{
    Object.entries(r.packs[slot]).forEach(([pieceStr,count])=>{
      const piece=+pieceStr; let matchedBase=null, qPerPiece=0;
  ...
  bases.forEach(b=>{
    const q=byBase[b]; const mk=(n)=>n?qToCell(n):"";
    const lines=[]; if(q.AM) lines.push(`Take ${qToPhrase(q.AM)} in the morning`);
    ...
    rows.push({ strengthLabel:`${r.med} ${b} mg ${/Tablet$/i.test(r.form)?"Tablet":formLabelCapsSR(r.form)}`, instructions:lines.join("\n"), am:mk(q.AM), mid:mk(q.MID), din:mk(q.DIN), pm:mk(q.PM) });
  });
  return rows;
}
...
function renderStandardTable(rows){
  ...
  rows.forEach((r,rowIdx)=>{
    // Skip empty non-final rows
    if(!(r.stop||r.review)){
      const any=["AM","MID","DIN","PM"].some(k=>r.packs && Object.keys(r.packs[k]||{}).length);
      if(!any) return;
    }

    if(r.stop || r.review){
      const tr=document.createElement("tr"); if((rowIdx%2)===1) tr.style.background="rgba(0,0,0,0.06)";
      tr.appendChild(td(r.date)); tr.appendChild(td("")); tr.appendChild(td(r.stop?"Stop.":"Review ongoing plan with the doctor.","instructions-pre"));
      tr.appendChild(td("","center")); tr.appendChild(td("","center")); tr.appendChild(td("","center")); tr.appendChild(td("","center"));
      tbody.appendChild(tr); return;
    }

    const cls=r.cls, packs=r.packs;

    if(cls==="Benzodiazepines / Z-Drug (BZRA)" || (cls==="Antipsychotic" && !/\bSR\b|Slow\s*Release/i.test(r.form))){
      const lines=perStrengthRowsFractional(r);
      lines.forEach((ln,i)=>{ const tr=document.createElement("tr"); if((rowIdx%2)===1) tr.style.background="rgba(0,0,0,0.06)";
        tr.appendChild(td(i===0? r.date : "")); tr.appendChild(td(ln.strengthLabel));
        tr.appendChild(td(ln.instructions,"instructions-pre"));
        tr.appendChild(td(ln.am,"center")); tr.appendChild(td(ln.mid,"center")); tr.appendChild(td(ln.din,"center")); tr.appendChild(td(ln.pm,"center"));
    ...
    const mgList=Array.from(allMg).sort((a,b)=>a-b); if(mgList.length===0) return;

    mgList.forEach((mg,i)=>{
      const tr=document.createElement("tr"); if((rowIdx%2)===1) tr.style.background="rgba(0,0,0,0.06)";
      tr.appendChild(td(i===0? r.date : ""));
      const am=packs.AM[mg]||0, mid=packs.MID[mg]||0, din=packs.DIN[mg]||0, pm=packs.PM[mg]||0;
      const instr=[]; if(am) instr.push(`Take ${am===1?"1":am} ${am===1?"tablet":"tablets"} in the morning`);
      ...
      tr.appendChild(td(am?String(am):"","center")); tr.appendChild(td(mid?String(mid):"","center"));
      tr.appendChild(td(din?String(din):"","center")); tr.appendChild(td(pm?String(pm):"","center"));
      tbody.appendChild(tr);
    });
  });

  table.appendChild(tbody); schedule.appendChild(table);
}
...
function renderPatchTable(rows){
  ...
  rows.forEach((r,rowIdx)=>{ const tr=document.createElement("tr"); if((rowIdx%2)===1) tr.style.background="rgba(0,0,0,0.06)";
  ...
  table.appendChild(tbody); patch.appendChild(table);
}

/* footer blurb */
function setFooterText(cls){
  const exp = { Opioid:"Expected benefits: Improved function and reduced opioid-related harms.",
    ...
    Antipsychotic:"Expected benefits: Lower risk of metabolic/extrapyramidal adverse effects." }[cls] || "—";
  const wdr = { Opioid:"Withdrawal: transient pain flare, cravings, mood changes.",
    "Benzodiazepines / Z-Drug (BZRA)":"Withdrawal: insomnia, anxiety, irritability.",
    "Proton Pump Inhibitor":"Withdrawal: rebound heartburn.",
    Antipsychotic:"Withdrawal: sleep disturbance, anxiety, return of target symptoms." }[cls] || "—";
  $("expBenefits").textContent=exp; $("withdrawalInfo").textContent=wdr;
}

/* printing/pdf – output card only */
