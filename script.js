...
function canQuarter(med, form){
  ...
  if(med==="Zolpidem" && /Slow Release/i.test(form)) return false;
  if(isMR(form)) return false;
  // Per your instruction: BZRA and Antipsychotics may be quartered (IR only)
  return true;
}
...
function parseMgFromStrength(str){
  if(!str) return 0;
  const lead = String(str).split("/")[0]; // ignore naloxone part
...
function populateClasses(){ const el=$("classSelect"); el.innerHTML=""; CLASS_ORDER.forEach(c=>{ const o=document.createElement("option"); o.value=c; o.textContent=c; el.appendChild(o); }); }
...
function distributeToAMPM_withFloor(totalTargetMg, med, form){
  ...
  // Guarantee floors if we can
  if(remaining >= 2*min && min>0){
  ...
  for(const p of pieces){
    while(remaining >= p - 1e-9){
      ...
      const slot=(pmMg <= amMg) ? "PM" : "AM"; // bias PM to be >= AM
      if(slot==="PM") pm[p]=(pm[p]||0)+1; else am[p]=(am[p]||0)+1;
...
function opioidStep(packs, percent, med, form){
  ...
  if(packTotalMg(packs.MID)>0 || packTotalMg(packs.DIN)>0){
    ...
    if(rem>0){
      // remove from AM/PM but keep BID until ≤ 2*min
      const min=smallestPiece(med, form) || 0;
      while(rem>0.0001 && (packTotalMg(packs.AM)>0 || packTotalMg(packs.PM)>0)){
        // choose slot with larger mg
        let slot = (packTotalMg(packs.PM) >= packTotalMg(packs.AM)) ? "PM" : "AM";
        ...
        for(const p of sizes){
          ...
          const canRemove = (newSlot >= min) || (newTotalBID < 2*min + 1e-6);
          if(canRemove){
    ...
    // only AM/PM doses → compute target and (re)distribute with PM ≥ AM
...
function buildInstructionLine(pack, tail){
  ...
  entries.forEach(({mg,c})=>{
    let name="tablet";
    if(!(cls==="Opioid" || cls==="Proton Pump Inhibitor")){
...
function strengthLabelsFromPacks(packs, med, form){
  ...
  [packs.AM,packs.MID,packs.DIN,packs.PM].forEach(pack=>{
    Object.keys(pack).forEach(mgStr=>{
      ...
      labels.add(`${med} ${(+mg.toFixed(2)).toString().replace(/\.00$/,'')} mg ${form.toLowerCase()}`);
    });
  });
...
function buildPlanTablets(){
  ...
  // Phase 1: first reduction happens before first printed week
  doOneStep(p1Percent);
  pushRow();
  ...
  // Forced terminal pattern per class
  const min=smallestPiece(med, form);
  if(min){
    if(cls==="Proton Pump Inhibitor"){
      // hold lowest at dinner, then stop
      ...
      // ensure BID lowest first, then nocte lowest, then stop
      date=addDays(date, p1Interval);
      packs={ AM:{[min]:1}, MID:{}, DIN:{}, PM:{[min]:1} }; pushRow();
      date=addDays(date, p1Interval);
      packs={ AM:{}, MID:{}, DIN:{}, PM:{[min]:1} };       pushRow();
      date=addDays(date, p1Interval);
      rows.push({ date: fmtDate(date), strengths: [], instructions: "Stop.", am:0, mid:0, din:0, pm:0 });
    } else {
      // default: nocte lowest then stop
      date=addDays(date, p1Interval);
      packs={ AM:{}, MID:{}, DIN:{}, PM:{[min]:1} }; pushRow();
      date=addDays(date, p1Interval);
      rows.push({ date: fmtDate(date), strengths: [], instructions: "Stop.", am:0, mid:0, din:0, pm:0 });
...
function buildPlanPatch(){
  ...
  // hold smallest one interval, then stop
  date=addDays(date,p1Interval);
...
function init(){
  ...
  $("savePdfBtn").addEventListener("click", ()=>{
    ...
    if(typeof html2pdf==="function"){
      html2pdf().set({ filename:'taper_plan.pdf', margin:10,
