...
const HALVABLE = new Set([
  ...
  "Haloperidol","Olanzapine","Zopiclone","Zolpidem" // SR handled below
]);

/* ===== Guidance text (shown before controls; also drives defaults) ===== */
const GUIDANCE = {
  "Benzodiazepines / Z-drugs": {
    text: "Typical: reduce ~25% every 2 weeks; near the end, slow to ~12.5% every 2 weeks. Consider sleep hygiene/CBT. Monitor and slow if needed.",
    preset: { p1Percent:25, p1Interval:14, p2Percent:12.5, p2Interval:14 }
  },
  "Antipsychotic": {
    text: "Common: reduce about 25–50% every 1–2 weeks with close monitoring for return of target symptoms; slower taper is reasonable.",
  ...
  "Opioid": {
    text: "If used < 3 months: reduce 10–25% weekly. If used > 3 months: reduce 10–25% every 4 weeks. We prefill 25% × 3 cycles, then 10% ongoing.",
    // dynamic preset set by duration below
  }
};
...
/* Duration visibility: only relevant for Opioid per your rule */
function updateDurationVisibility(){
  const show = ($("classSelect").value==="Opioid");
  $("durationWrap").style.display = show ? "" : "none";
}

/* Best-practice + default prefill (class + opioid duration) */
function updateBestPracticeAndPrefill(){
  ...
  // Opioid: set dynamic defaults from duration
  if(cls==="Opioid"){
    if(duration==="<3"){
      // 25% weekly ×3, then 10% weekly
      preset = { p1Percent:25, p1Interval:7, p2Percent:10, p2Interval:7 };
      text += " Prefill: 25% every 1 week for 3 cycles, then 10% weekly.";
      // we'll stop Phase 1 at week 3 automatically when building plan
      $("p1StopWeek").value = 3;
    } else if(duration===">3"){
      ...
      // if no duration chosen yet, leave controls blank until user selects
      $("p1StopWeek").value = "";
    }
  }
  ...
  // Prefill controls (user can edit)
  if(preset){
...
/* ===== Build plan (two-phase with optional stop-at-review) ===== */
function buildPlan(){
  const cls = $("classSelect").value;
  const med = $("medicineSelect").value;
  const form= $("formSelect").value;
  ...
  // Phase 1 (respect p1StopWeek if provided)
  while(target > 0){
    pushWeekRow(target);
  ...
  // End row (alternate days / PRN for BZRA/Z-drugs, PPI, AP; opioids continue to 0)
...
/* ===== Patch schedule (fentanyl q3d, buprenorphine q7d) ===== */
function renderPatchTable(weeks, intervalDays, startDate, timeStr, meta){
  ...
  // Build a sequence of application events using target dose in effect at that date.
  const startTimeParts = (timeStr||"09:00").split(":").map(n=>parseInt(n||0,10));
  const startDT = new Date(startDate);
  startDT.setHours(startTimeParts[0]||9, startTimeParts[1]||0, 0, 0);

  // Determine plan end ~ through the final week row
  ...
  for(let t = new Date(startDT); t <= endDT; t = addDays(t, intervalDays)){
    // Find the active week for this datetime
    ...
    events.push({
      apply: new Date(t),
      remove: addDays(new Date(t), intervalDays),
      comps
    });
  }
  ...
  events.forEach(ev=>{
    ...
    // Use the first component for image lookup if present
...
function findActiveWeek(weeks, dateTime){
  // Find latest week row whose date <= dateTime
