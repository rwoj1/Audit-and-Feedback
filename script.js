...
function stepBZRA(packs, percent, med, form){
  ...
  // ---- Read selected strengths (fallback to catalogue) as numeric mg ----
  const strengths = getSelectedStrengthsBZRA("Benzodiazepines / Z-Drug (BZRA)", med, form);
  if (!strengths.length) return packs; // safety

  // ---- Build allowed unit options (full tablets + single halves where permitted) ----
  const unitOpts = buildAllowedUnits(med, form, strengths); // [{unitMg, sourceMg, piece:1|0.5}, ...]
  if (!unitOpts.length) return packs;

  // ---- End principle (daily variant): Stop at drug min if achievable; otherwise Review at lowest achievable ----
  const minDaily = minDailyFor(med, form);              // numeric mg
  const achievableMin = minAchievable(unitOpts);        // lowest non-zero achievable dose with current selection
  const canHitMinDaily = (minDaily > 0 && achievableMin > EPS && Math.abs(minDaily - achievableMin) < 1e-6);

  // If we are already at the min boundary, set flag for next step and hold this dose
  if (canHitMinDaily && Math.abs(tot - minDaily) < EPS) {
    // Hold one phase at min, then Stop on next tick
    if (typeof window !== "undefined") window._forceStopNext = true;
    return packPMTo(minDaily, unitOpts); // keep min for this step
  }
  if (!canHitMinDaily && Math.abs(tot - achievableMin) < EPS) {
    if (typeof window !== "undefined") window._forceReviewNext = true;
    return packPMTo(achievableMin, unitOpts); // hold lowest achievable, then Review
  }

  // ---- Compute next target from previous achieved ----
  const rawTarget = tot * (1 - percent/100);

  // Quantise to nearest multiple of the smallest allowed unit; tie → FEWEST PIECES, then ROUND UP
  const step = Math.min.apply(null, unitOpts.map(u => u.unitMg));
  const down = Math.max(0, step * Math.floor(rawTarget / step));
  const up   = step * Math.ceil(rawTarget / step);
  const cands = (down === up) ? [down] : [down, up];

  // Ensure progress: if quantising lands on current dose, nudge down by one step
  let bestS = cands[0];
  let bestPack = null;
  let bestScore = null;

  for (const S of cands) {
    const P = packPMTo(S, unitOpts);
    if (!P) continue;
    const pieces = totalPieces(P.PM); // sum of tablet pieces; 1 tab = 1.0, half = 0.5
    const diff = Math.abs(S - rawTarget);
    const roundedUp = (S >= rawTarget) ? 1 : 0;
    const score = [diff, pieces, -roundedUp, S]; // lexicographic: nearest → fewest pieces → prefer round up → lower S (stable)

    if (!bestScore || lexLess(score, bestScore)) {
      bestS = S; bestPack = P; bestScore = score;
    }
  }
  // No viable pack (rare) → stay unchanged
  if (!bestPack) return packs;

  // If no progress, force one step down
  if (Math.abs(bestS - tot) < EPS) {
    const S2 = Math.max(0, bestS - step);
    const P2 = packPMTo(S2, unitOpts);
    if (P2) { bestS = S2; bestPack = P2; }
  }

  // After we move, check if the *new* dose is exactly at a boundary → set next-step flag
  if (canHitMinDaily && Math.abs(bestS - minDaily) < EPS) {
    if (typeof window !== "undefined") window._forceStopNext = true; // Stop after this phase
  } else if (!canHitMinDaily && Math.abs(bestS - achievableMin) < EPS) {
    if (typeof window !== "undefined") window._forceReviewNext = true; // Review after this phase
  }
  ...
  function getSelectedStrengthsBZRA(cls, med, form){
    // Prefer explicit selection
    try {
      if (typeof selectedProductMgs === "function") {
        const picked = selectedProductMgs(); // e.g., [1, 2] or ["1 mg", "2 mg"]
        if (Array.isArray(picked) && picked.length) {
          return picked.map(toMgLoose).filter(n=>Number.isFinite(n) && n>0).sort((a,b)=>a-b);
        }
      }
    } catch(_) {}
    // Fallback to picker / catalogue
    try {
      if (typeof strengthsForPicker === "function") {
        const raw = strengthsForPicker(cls, med, form) || [];
        return raw.map(toMgLoose).filter(n=>Number.isFinite(n) && n>0).sort((a,b)=>a-b);
      }
    } catch(_) {}
    // Last resort: scan CATALOG
    try {
      if (window.CATALOG && CATALOG[cls] && CATALOG[cls][med]) {
        const node = CATALOG[cls][med];
        const list = node[form] || Object.values(node).flat();
        return (list||[]).map(toMgLoose).filter(n=>Number.isFinite(n)&&n>0).sort((a,b)=>a-b);
      }
    } catch(_) {}
    return [];
  }
  ...
  function canHalve(med, form, mg){
    const name = String(med||"").toLowerCase();
    const fr   = String(form||"").toLowerCase();
    // Never halve SR / Slow Release / ODT / wafer / dispersible
    if (/slow\s*release|sr|cr|er|mr|odt|wafer|dispers/i.test(fr)) return false;
    // Alprazolam 0.25 mg must not be halved
    if (name.includes("alprazolam") && Math.abs(mg - 0.25) < 1e-6) return false;
    // Zolpidem SR: never halve
    if (name.includes("zolpidem") && /slow\s*release|sr|cr|er|mr/.test(fr)) return false;
    return true;
  }

  function buildAllowedUnits(med, form, mgList){
    const out = [];
    for (const mg of mgList){
      // full tablet
      out.push({ unitMg: mg, sourceMg: mg, piece: 1.0 });
      // half tablet (single halving only) if allowed
      if (canHalve(med, form, mg)) {
        out.push({ unitMg: mg/2, sourceMg: mg, piece: 0.5 });
      }
    }
    // dedupe by unitMg (keep the one that uses the largest sourceMg to minimise small fragments)
    const bestByUnit = new Map();
    for (const u of out){
      const k = u.unitMg.toFixed(6);
      const prev = bestByUnit.get(k);
      if (!prev || u.sourceMg > prev.sourceMg) bestByUnit.set(k, u);
    }
    return Array.from(bestByUnit.values()).sort((a,b)=>a.unitMg-b.unitMg);
  }

  function minDailyFor(med, form){
    const name = String(med||"").toLowerCase();
    const fr   = String(form||"").toLowerCase();
    // SR zolpidem has SR-specific min
    if (name.includes("zolpidem") && /slow\s*release|sr|cr|er|mr/.test(fr)) return 6.25;
    // Base mins per drug (IR or plain tablets)
    if (name.includes("alprazolam"))   return 0.25;
    if (name.includes("clonazepam"))   return 0.25;
    if (name.includes("diazepam"))     return 1.0;
    if (name.includes("flunitrazepam"))return 0.5;
    if (name.includes("lorazepam"))    return 0.5;
    if (name.includes("nitrazepam"))   return 2.5;
    if (name.includes("oxazepam"))     return 7.5;
    if (name.includes("temazepam"))    return 5.0;
    if (name.includes("zolpidem"))     return 5.0; // IR tablet
    if (name.includes("zopiclone"))    return 3.75;
    return 0;
  }

  function minAchievable(unitOpts){
    // lowest non-zero unit we can represent while respecting a practical max of 4 tablets (pieces can sum to <= 4)
    const minUnit = Math.min.apply(null, unitOpts.map(u=>u.unitMg));
    return (minUnit > 0) ? minUnit : 0;
  }

  function totalPieces(pmMap){
    // pieces = full tablets (1.0) + halves (0.5) …
    return Object.values(pmMap||{}).reduce((s,c)=>s + (c||0), 0);
  }

  function packPMTo(targetMg, unitOpts){
    // Greedy largest-first in unit mg, with a soft cap of 4 tablets (sum of pieces ≤ 4)
    if (!(targetMg > EPS)) return { AM:{}, MID:{}, DIN:{}, PM:{} };
    const opts = unitOpts.slice().sort((a,b)=>b.unitMg - a.unitMg);
    ...
    for (const u of opts){
      if (r <= EPS) break;
      let q = Math.floor(r / u.unitMg + 1e-9); // how many of this unit
      while (q > 0 && pieces + q * u.piece > 4 + 1e-9) q--; // enforce cap
      if (q > 0){
        PM[u.sourceMg] = (PM[u.sourceMg] || 0) + q * u.piece;
        r -= q * u.unitMg;
        pieces += q * u.piece;
      }
    }
    // Fill any tiny remainder with the smallest unit if still under cap
    if (r > EPS) {
      const smallest = opts[opts.length-1];
      if (pieces + smallest.piece <= 4 + 1e-9 && r <= smallest.unitMg + 1e-9) {
        PM[smallest.sourceMg] = (PM[smallest.sourceMg] || 0) + smallest.piece;
        r -= smallest.unitMg;
        pieces += smallest.piece;
      }
    }
    if (r > EPS) return null; // could not represent exactly
